...
import subprocess
import json
import clr  # Required to load .NET assemblies in IronPython
...
class PrintSheetsWindow(forms.WPFWindow):
    def __init__(self, xaml_file_name):
        ...
        self.FolderPath.Text = self._save_location
        os.environ['selectedPath'] = self._save_location


        print("Initial Save Location: {}".format(self._save_location))
    ...
    def on_browse_button_click(self, sender, e):
        ...
        if result == DialogResult.OK:
            # Update save location dynamically based on user input
            self._save_location = dialog.SelectedPath
            self.FolderPath.Text = self._save_location  #new line
            os.environ['selectedPath'] =  self._save_location
            print("New Save Location: {}".format(self._save_location))
    ...
    def save_with_pdf24(self, full_path):
        ...
        try:
            # Example for PDF24 saving command
            pdf24_path = r'C:\Program Files\PDF24\pdf24.exe'  # Path to PDF24 executable
            command = [
                pdf24_path,
                '--output', full_path,  # Destination path for the saved PDF
                '--auto-save',           # PDF24 auto-save flag
            ]

            # Print the command for debugging
            print("Running command: {}".format(" ".join(command)))

            # Execute the command to save the PDF at the desired location
            subprocess.run(command)
            print("PDF saved to {}".format(full_path))  # Using .format() for compatibility
